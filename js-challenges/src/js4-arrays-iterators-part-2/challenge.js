...
export const totalScoresArr = (scoresArr) => {
  const totalScore = scoresArr.reduce((acc, curr) => {
    return acc + curr;
  }, 0);
...
export const reverseString = (toReverse) => {
  ...
  const reverseStr = str.reduce((acc, curr) => {
    acc.unshift(curr);
    return acc;
  }, []);
...
export const sortCharactersAlphabetically = (charcterArr) => {
  const arrangeArr = charcterArr.map((char) => {
    return char.toLowerCase();
  });

  return arrangeArr.sort();
};
...
export const checkItemInstock = (toCheck) => {
  ...
  const stocks = stockList.filter((item, index) => {
    if (item.includes(toCheck)) {
      return `${item} is in stock, it is on aisle ${index}`;
    } else {
      return `Sorry ${item} is not in stock.`;
    }
  });
...
export const checkPrimaryColours = (coloursArr) => {
  ...
  const allPrimary = coloursArr.every((colour) => {
...
export const checkStringPalindrome = (stringOne) => {
  // const stringOneArr = Array.from(stringOne);
  // const palindrome = stringOneArr.every((element, index) => {
  //   return element === stringOneArr[stringOneArr.length - 1 - index];
  // });
  // return palindrome;
...
export const totalNestedScoresArr = (scoresArr) => {
  const sumArr = scoresArr.reduce((acc, curr) => {
    const sumCurr = curr.reduce((ac, cu) => {
      return ac + cu;
    }, 0);
...
export const encryptString = (toEncrypt) => {
  // const strArr = toEncrypt.reduce((acc, curr)) => {
};
